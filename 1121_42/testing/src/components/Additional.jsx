...
export default function Additional() {
  ...
  //   기존에 useEffect를 사용할 경우 어떤 상태(a)의 값이 변화시 a를 기준으로 다른 상태(b)의 값을 계산하여 정의하는 경우가 있다. (ex. axios를 통한 데이터 받기)
  ...
  //   즉 testFunc은 함수이므로 호출할때는 testFunc()으로 사용해야한다.
  ...
  // 위와 같이 function 등등으로 함수를 선언했을 때는 리랜더링 될 때마다 함수를 다시 정의한다.
  // useCallback 사용시 두번째 매개변수에서 상태값이 변화하지 않으면 재정의 하지 않고 있던 것을 사용한다.
  // 즉, 필요 없는 코드의 실행을 막을 수 있기 떄문에 최적화시 사용한다.
  // userMemo또한 마찬가지로 최적화 시 사용한다
