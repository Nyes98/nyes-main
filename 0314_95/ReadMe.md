...
contract nftToken is ERC721Enumerable, Ownable {
  ...
  // 민팅 가격, 사용자가 NFT를 올릴 때마다 1ether씩 받는다.
  // ether, gwei, second, minute, day 등 여러 단위를 사용할 수 있다.
  // ether : 10 ** 18
  ...
  function getRandomTokenData(
    ...
    // 유일한 값인 tokenId를 가져와서 암호화한 후 나머지 연산으로 0~99까지 랜덤한 수를 만든다.
...
contract SaleToken {
  ...
  function SalesToken(uint _tokenId, uint _price) public {
    ...
    // OpenSea 기준으로 setApprovedForAll 메서드가 이미 있다.
    //  - 메타마스크 연결 / 로그인 시 => 메타마스크의 계정에 대한 권한을 위임 받는다.(서명)
  ...
  function getSaleTokenList() public view returns (TokenInfo[] memory) {
    ...
    // [Javascript] let list = new Array(SaleTokenList.length)
...
- 민팅 => NftToken의 mintToken을 호출한다. (이더도 보내야한다.)
...
- 구현 시나리오에 따라 다르지만 (만약 민팅 후 NFT 정보 페이지로 이동 또는 등록한 NFT 정보를 모달창으로 출력하여 확인할 경우) getLatestToken을 사용하게 된다.
