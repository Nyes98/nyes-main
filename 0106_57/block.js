...
class Block extends IBlock {
  ...
  create(_previousBlock, _adjustmentDifficulty, _adjustmentTimestamp, _data) {
    try {
      ...
      //   throw 명령어를 사용해서 try문을 멈추가 catch로 입력값을 전달한다.
      // merkleRoot에서 오류 발생시 생성 멈춤
  ...
  getMerkleRoot(_data) {
    ...
    return Array.isArray(_data)
      ...
      : { isError: true, error: "Error" }; // 배열이 아니다
  }
  ...
  createHash(_block) {
    // hash 생성 함수
    return SHA256(
      // SHA256 방식의 hash 암호화 사용, 256은 256bits를 뜻한다.
      Object.entries(_block)
        // 객체의 키와 값을 배열로 변경 [[key,value], [key,value]]
        .filter((item) => item[0] !== "hash" && item[0] !== "data")
        // hash 생성 메서드이기 떄문에 hash를 제외, data는 merkleRoot로 대체됨
        .join("")
      //   배열을 하나의 문자열로 연결
    );
  }
  ...
  getDifficulty({
    ...
    // 높이의 난이도 조절 단위 개수 나머지가 0이 아니면 개수가 맞아 떨어지지 않아서 이전 난이도를 이용
    if (height % DIFFICULTY_ADJUSTMENT_INTERVAL !== 0)
      return previousDifficulty;
    //   만약 난이도 조절 단위 개수가 10이라면 10개 단위로 난이도 조절을 하게 된다.
    // 높이가 9까지는 0, 높이가 10~19 1, 이후에는 각 10 단위마다 아래 코드로 난이도를 결정
    ...
    //   설정된 블록 10개 생성 시간보다 0.5배 미만이면 난이도 증가

    // 난이도 감소
    if (timeTaken > timeExpected * 1.5) return adjustmentDifficulty - 1;
    // 설정된 블록 10개 생성 시간보다 1.5배 초과이면 난이도 감소
  ...
  isValidNewBlock(_newBlock, _previousBlock) {
    ...
    if (_newBlock.height !== _previousBlock.heigth + 1) {
      // 생성된 블록이 이전 블록의 다음 개수를를 가져오는지 확인
