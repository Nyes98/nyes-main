...
// 요청과 응답 << 요청이 없으면 서버가 응답을 보낼 수 없다. 즉, 서버가 클라이언트에게 마음대로 데이터나 정보 등을 보낼 수 없다.
...
module.exports = (server) => {
  ...
  //   소켓을 연결한다, 아무것도 안 뜨더라도, 확인이 안되더라도 연결은 계속 되어있다.
  let count = 0;
  socket.on("connection", (ws, req) => {
    ...
    ws.interval = setInterval(() => {
      ws.send(count++);
      //   데이터를 전송,보낸다. 한쪽에서 send로 보내면 다른 쪽에선 on으로 받는다.
    }, 1100);
    ws.on("close", () => {
