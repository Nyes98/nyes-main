...
// 배열은 배열인데 내부에서 중복을 처리해주는 배열 (배열이랑은 다르다, 쉽게 설명하려고 배열이라 한 것)
// 사용할 때는 new Set()을 변수에 정의하여 변수를 사용한다. (let a = [])
// clear(), add(), size
...
function reset() {
  ...
  console.log(comSel);
  alert("컴퓨터는 준비를 마쳤다. 맞춰봐.");
}

function selectNum() {
  if (!comSel.length) return;
  // 컴퓨터가 준비되지 않은 상태에서(숫자가 선택되지 않음) 게임을 진행하지 않도록 클릭 함수 멈춤
  let playerSel = "";
  let strike = 0;
  let ball = 0;
  ...
  while (playerSel.length !== 3) {
    ...
    if (!playerSel) return alert("포기하겠습니까?");
    ...
    // playerSel의 NaN이 있으면 중간에 숫자가 아닌 문자가 있기 때문에
    // 마찬가지로 playersel을 빈 값으로 정의한다.
  }
  ...
  comSel.forEach((item, index) => {
    ...
    // 아이템과 playerSel의 index 번 째 아이템이 같은가? 같으면 strike++
    else if (playerSel.includes(item)) ball++;
    // 아이템이 playerSel의 포함되어 있는가? 있으면 ball++
  });
  ...
  //   playerSel = "";  << 굳이 하지 않아도 됨 {while에 걸려서 그만두려고}
  // 플레이어의 선택 삭제
  if (strike === 3) {
