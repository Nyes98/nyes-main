...
[...divs].forEach((elem) => {
  ...
  // 보통 [...***]을 사용하여 유사 배열 ***을 배열로 변환한다. << 해당 방법 사용 시 직전 코드 마지막에 ;(세미콜론)이 없으면 한 줄(하나의 코드)로 인식돼서 오류가 발생하는 경우가 있다.
  // forEach는 배열의 각 아이템을 매개변수함수에 매개변수로 전달하여 함수를 호출한다.
...
// 캡쳐링은 addEventListener를 사용해서 확일할 수 있다.
...
// 34~36번 줄처럼 선언(초기화)하기 전에 호출해서(가져와서) 사용하는 현상을 호이스팅 이라고 한다.
// 공식적으로는 hoisting은 var를 사용 시 Javascript가 최상단에서 미리 선언하는 것을 말한다.
// var, function으로 가능하다.
// hoisting은 좋은 기능이 아니기 때문에 사용하지 않는 것을 권장한다. (var 사용 금지)

function hoisting() {
  console.log(++a);
}

// expressionsHoisting(); << 위에서하면 오류 (hoisting 오류)
...
// arrowHoisting(); << 위에서하면 오류 (hoisting 오류)
