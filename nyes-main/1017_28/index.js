...
// npm : node package manager - 라이브러리
// 서버는 외부에서 접속하는 컴퓨터
// port : 일종의 문이다.
// HTTP : 80 포트 사용
// HTTPS : 443 포트 사용
...
// 싱글 스레드 : 한번에 여러 작업을 실행하지 못한다. (크롬에서 탭을 나누면 스레드는 최소 하나씩 가져간다.)
// Javascript는 싱글 스레드이다. 다만, 멀티 스레드기능을 사용할 수 있게 해주는 라이브러리가 있다.

// 논 블로킹 I/O => 동기/비동기, 블로킹/논블로킹 In Out (입출력)
// 동기 : 실행되는 작업이 서로 연관성을 가진다. => 하나가 실행이 완료된 후에 그 반환값을 사용해 다른 작업을 실행한다. => 이전 작업을 기다려야 한다.
const a = 1;
const b = 2;
function add(a, b) {
  return a + b;
...
// 블로킹, 논블로킹
// 블로킹 : 동기 작업은 이전 작업을 기다려야 한다 => 이전 작업이 끝날 때까지 멈춰야한다.
// 논블로킹 : 이전 작업과 무관하게 작업이 진행된다, 멈추지 않는다.
// 블로킹과 동기 방식으로 계속 코드를 써왔다.

// 싱글 스레드는 하나의 작업만을 진행하는데 어떻게 논 블로킹이 성립되는가?
// 답은 시분할(라운드 로빈)
// 시분할 : 시간 분할, 코드를 일정 시간으로 잘라서 실행한다.
// 여러 작업을 할 때, 각 코드에 대해서 일정한 시간을 진행 후 다른 코드를 진행한다. 한번 다 돌린 후에 다시 이어서 진행한다.
