...
function addFunc(firstNum, secondNum) {
  // firstNum, secondNum가 매개변수이다.
  console.log(firstNum + secondNum);
}

addFunc(2, 3);
// firstNum : 2, secondNum : 3 즉, 2+3=5가 출력된다.

function addFunc2(firstNum, secondNum) {
  return firstNum + secondNum;
  // return : 되돌린다, 반환값, addFunc2가 끝났을 때 함수가 돌려주는 값이다.
}
...
answer = addFunc(56, 3);
console.log(answer);
// addFunc 함수는 return이 없기 때문에 asnwer 변수에 '정의되지 않았다'를 뜻하는 undefined가 정의된다.

console.log(Math.random());
console.log(console.log("asdf"));
console.log(console.log(addFunc2(1, 2)));
// 제일 밖의 콘솔 로그를 실행 -> 매개변수로 2번째 콘솔 로그
// 2번째 콘솔 로그 실행 -> 매개변수로 addFunc2
// addFunc2 실행 -> return = 3
// 2번째 콘솔로그가 3을 로그 출력
...
do {
  ...
  // sel이 반환한 값을 palyerSel에 다시 정의한다.(재정의)
