...
// 2. action이 함수인지 확인하여 함수라면 reducer에 전달하는 것이 아닌 action 함수 자체를 호출한다.
// - 확인은 thunk가 해준다. 전달하는 action 함수는 async, await가 가능하다.
// 3. action이 객체인 경우 기존의 redux 실행 순서와 마찬가지로 작동한다.
// 순서 총괄 : action -> dispatch -> action -> dispatch -> reducer

const asyncIncrement = () => {
  // 2. 사용자 클릭으로 dispatch(action.asnycIncrement())가 호출된다.
  console.log("2. asyncIncrement 호출");
  return async (dispatch, getState) => {
    ...
    try {
      ...
      // 7. result를 받아 dispatch를 호출해 action으로 전달한다.
      console.log("7. result = ", result);
      dispatch({ type: result });
      // return await promiseTime(TYPE.INCREMENT, 1000); << state에 적용되지 않는다.
    } catch (error) {
...
export const reducer = (state = initialize, action) => {
  ...
  switch (type) {
    ...
    case TYPE.INCREMENT:
      // 10. return이 된 state+1을 state(현재는 count)에 적용한다/
