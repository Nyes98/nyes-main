  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        resolve(type);
        // App.js의 data로 type이 전달된다.
      }, time);
      //   매개변수로 전달된 시간 후에 매개변수로 전달된 type을 반환한다.
      //   만약 서버와 통신(axios 등)할때 setTimeout이 아니라 axios의 then을 이용해서 resolve 메서드를 호출하자.
    } catch (error) {
...
// 비동기 처리 : 다른 작업과 함께 진행하는 방식
//  - setTimeout과 같이 작업에 대해서 예약해둔다.
// 동기 처리 : 실제 코드를 읽듯이 순서대로 처리
//  - 일반 코드와 같다
...
// 비동기 처리를 연속되게 실행할 때
// 1초마다 한번씩 실행한다. 아래 코드는 콜백지옥이라고 한다.
// setTimeout(() => {
//   console.log("비동기 확인1");
//   setTimeout(() => {
//     console.log("비동기 확인2");
...
const callbackPromise = (text, time) => {
  return new Promise((resolve, reject) => {
    // resolve는 성공시 resolve의 매개변수로 결과를 전달한다.
    // reject는 실패시 에러를 전달한다.
    try {
      // try 스코프 내의 코드를 실행한다.
      // 도중에 오류가 발생하면 아래의 catch의 error로 전달한다.
      //   if (text == "프라미스 4") reject("end");
...
//   .then((result) => {
//     // then은 promise 성공시 실행하는 콜백함수를 전달한다.
//     // 콜백함수의 매개변수는 76번째 줄의 resolve에 전달된 매개변수(text)를 result로 받는다.
//     console.log(result);
//     return callbackPromise("프라미스 2", 1000);
//     // 다음 프라미스(작업)은 return으로 반환하며 아래의 then으로 위의 프라미스(89)의 결과를 받을 수 있다.
//   })
//   .then((result) => {
//     console.log(result);
