...
- Metadata란
  - 데이터를 위한 데이터다, 테이터에 대한 데이터다.
  - 데이터에 관한 구조화된 데이터, 다른 데이터를 설명해주는 데이터
    - 사진을 찍으면 메타데이터 : [언제찍었고, 용량이 몇이고 등]
    - 필터를 구분짓는 데이터
...
interface IERC721Metadata{
  fucntion name() external view returns(string memory);
...
- URI : Uniform Resource Identifier의 약자로 특정 데이터를 식별하는 식별자, 리소스를 구분하는 고유 문자열
...
contract ERC721 is IERC721, IERC721Metadata {
  ...
  function approve(address _to, uint _tokenId) external override {
    address owner = _owners[_tokenId];
    // address owner = ownerOf(_tokenId); << 수수료가 필요하기 때문에 이렇게 사용하지 않는다.
    // 위임할 토큰의 주인 확인
    // ownerOf 호출 시 gas가 소모된다.
