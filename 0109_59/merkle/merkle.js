...
// .sync(data) : merkle Tree를 만들 데이터를 매개변수로 전달한다. data는 배열이여야한다.
...
const createMerkleRoot = () => {
  ...
  // .toString() : 문자열로 변환한다.
  // .toUpperCase() : SHA256을 쓸 경우 영어가 소문자로 나타나기 때문에 대문자로 수정해준다.
};
...
// 데이터의 모든 정보(아이템)를 한번씩 hash(SHA256)로 암호화한다.

const secondTreeRoot = SHA256(firstTree[0] + firstTree[1])
  .toString()
  .toUpperCase();
//   데이터 안의 2개의 아이템을 암호화한 후 연결(더)해서 다시 암호화해서 한개의 문자열로 만든다.

const secondTree = [];
for (let i = 0; i < firstTree.length; i += 2) {
  secondTree.push(
    SHA256(firstTree[i] + firstTree[i + 1])
      .toString()
      .toUpperCase()
  );
}
