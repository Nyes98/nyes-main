...
async function getBalancce(_account) {
  const {
    data: { result },
  } = await request({
    data: {
  ...
  balanceElem.innerHTML =
    parseInt(result, 16) / Math.pow(10, 15) / 1000 + " ETH";
  // balanceElem.innerHTML = parseInt(result, 16) / Math.pow(10, 18);
}

async function getWallet(_account) {
  if (interval !== undefined) mineStop();
  accountElem.innerHTML = _account;
  await getBalancce(_account);
...
async function getAccounts() {
  ...
  result.push("0x4cbAec90AC7ea69caA5173e9162897F5DE921f51");
  result.forEach((item) => {
...
document.getElementById("start").onclick = async function () {
  ...
  interval = setInterval(() => {
    getBalancce(accountElem.innerHTML);
  }, 2000);
};
